...
写 PRD 时从以下角度质疑需求的合理性：

- **技术边界** — 当前平台/框架的约束是否允许这个需求？（如终端不支持图片）
- **数据源** — 需要的数据 API 是否提供？没有的话是否有替代方案？
- **复杂度** — 这个需求的实际实现成本是否值得？有没有更简单的替代？

发现不合理的需求时，在 PRD Discussion 中用 `[设计备注]` 标注原因和建议方案。
